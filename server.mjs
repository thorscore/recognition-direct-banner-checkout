...
function buildAttributes(formData, artworkUrls) {
  return [
    ...
    attribute("Player Names", field(formData, "players", 1500)),
...
function buildYouthSportsBannerPrompt(formData, artworkUrls = {}) {
  ...
  const prompt = [
    "Create a bold, creative youth sports banner concept for Recognition Direct.",
    "Prioritize an exciting visual concept first. Do not make it look like a plain print layout or generic template.",
    "The goal is an eye-popping design that can later be rebuilt or cleaned up for print production.",
    "Use the exact customer text supplied below. Do not invent player names, player numbers, logos, mascots, or sponsors.",
    "If any requested detail is missing, leave it out cleanly and mention what is missing after the design concept.",
    "",
    ...
    "",
    promptLine("Player names", field(formData, "players", 2000)),
    ...
    "- Think like a sports poster artist first, then like a production designer second.",
    "- Make the team name the visual hero: oversized, bold, dimensional, and full of energy.",
    "- Use a dramatic mascot/action-style composition when it fits the team name.",
    "- Use dynamic sports graphics: stadium lights, flying motion, splashes, speed lines, smoke, turf/field texture, ball motion, shield shapes, comic-book energy, glowing highlights, and layered depth.",
    "- Match the sport with sport-specific visual elements such as field texture, stadium lights, ball motion, goal net, grass, action streaks, shield shapes, or dynamic diagonal panels.",
    "- Match the team colors without making the design one-note; use dark and light contrast so the text pops.",
    "- Make the design feel custom to the team name, not like a standard banner form.",
    "- Include the player names creatively, such as on balls, jerseys, cards, badges, stars, ribbons, or a strong lower roster strip.",
    "",
    ...
    "- Keep player names accurate and readable. Use names only and do not add player numbers.",
    ...
    "- Use the lower area for a clean roster section if that is the strongest design choice.",
    "- Leave room for sponsor logos or extra names only if requested.",
    "",
    "Output request:",
    "- First create the strongest creative banner concept image.",
    "- After the concept, include brief production notes for converting it into a print-ready 5 ft x 3 ft banner file.",
    "- Do not let print-production constraints make the first concept boring.",
