...
const CATEGORY_COLLECTIONS = [
  { title: "Banners", handle: "banners", tests: [/banner/, /tension-fabric$/, /pole-banner/] },
  { title: "Advertising Flags", handle: "advertising-flags", tests: [/flag/, /teardrop/, /feather/, /rectangle-flag/, /custom-pole/] },
  { title: "Banner Stands", handle: "banner-stands", tests: [/retractable/, /x-stand/, /poster-stand/, /banner-stand/, /tension-fabric-stand/, /podium/] },
  { title: "Rigid Signs & Magnets", handle: "rigid-signs-and-magnets", tests: [/coroplast/, /foam/, /pvc/, /aluminum-sign/, /aluminum-sandwich/, /magnet/, /gatorfoam/, /styrene/, /yard-sign/] },
  { title: "Adhesive Products", handle: "adhesive-products", tests: [/adhesive/, /vinyl/, /window-perf/, /floor-graphics/, /wall-graphics/, /frosted/, /etched/] },
  { title: "Wall Art", handle: "wall-art", tests: [/canvas/, /acrylic/, /framed-print/, /wood-frame/, /wall-mural/] },
  { title: "Real Estate", handle: "real-estate", tests: [/real-estate/] },
  { title: "A-Frames & Sign Holders", handle: "a-frames-sign-holders", tests: [/a-frame/, /signicade/, /simposign/, /snap-poster/, /sign-holder/] },
  { title: "Trade Show Products", handle: "trade-show-products", tests: [/seg/, /booth/, /backdrop/, /popup/, /pop-up/, /velcro-fabric/, /tension-fabric-display/] },
  { title: "Event Tents", handle: "event-tents", tests: [/tent/, /event-tent/, /tent-wall/] },
  { title: "Table Throws", handle: "table-throws", tests: [/table/, /throw/, /runner/] },
  { title: "Reflective Products", handle: "reflective-products", tests: [/reflective/] },
  { title: "Dry Erase Products", handle: "dry-erase-products", tests: [/dry-erase/] },
  { title: "Hardware", handle: "hardware", tests: [/hardware/, /stake/, /pole/, /base/, /stand-off/, /standoff/, /sandbag/, /carrying-bag/, /light-for-banner/, /grommet/] },
  { title: "Channel Letters", handle: "channel-letters", tests: [/channel-letter/] },
  { title: "DTF Transfers", handle: "dtf-transfers", tests: [/direct-to-film/, /dtf/, /uv-dtf/] },
  { title: "Materials & Supplies", handle: "materials-and-supplies", tests: [/film/, /lamination/, /masking-tape/, /double-sided/, /red-tape/, /canvas-fabric/, /material/, /rolls/, /polyester/] },
  { title: "Posters", handle: "posters", tests: [/poster/] },
];

function productKey(product) {
  return `${product.title} ${product.handle}`.toLowerCase().replace(/&/g, "and").replace(/[^a-z0-9]+/g, "-");
}

async function fetchCatalogProductsFromShopify() {
  const query = `#graphql
    query CatalogProducts($after: String) {
      products(first: 250, after: $after, query: "tag:catalog-migration") {
...
async function getOrCreateCustomCollection(collection) {
  const existing = await shopifyRest(`/custom_collections.json?limit=250&handle=${encodeURIComponent(collection.handle)}`);
  const match = (existing.custom_collections || []).find((item) => item.handle === collection.handle);
  if (match) return match;

  const created = await shopifyRest("/custom_collections.json", {
    method: "POST",
    body: JSON.stringify({
      custom_collection: {
...
async function fetchCollectionProductIds(collectionId) {
  const payload = await shopifyRest(`/collects.json?collection_id=${collectionId}&limit=250`);
  return new Set((payload.collects || []).map((collect) => Number(collect.product_id)));
}

async function handleSyncCategoryCollections(req, res, url) {
  if (url.searchParams.get("key") !== "sync-categories-2026-06-03") {
    return json(res, 404, { error: "Not found." });
  }

  const products = await fetchCatalogProductsFromShopify();
  const summary = [];

  for (const collection of CATEGORY_COLLECTIONS) {
    const customCollection = await getOrCreateCustomCollection(collection);
    const currentProductIds = await fetchCollectionProductIds(customCollection.id);
    const matches = products.filter((product) => collection.tests.some((test) => test.test(productKey(product))));
    let added = 0;
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/internal/sync-category-collections") return await handleSyncCategoryCollections(req, res, url);
